...
@WebServlet("/ReadHospital")
public class ReadHospitalServlet extends HttpServlet {
...
//				System.out.println(search);
				...
				List<HospitalDTO> data = hDAO.findList(currentPage, recordsPerPage, search);  //limit을 건 select 가져오기
...
//				int row = hDAO.getNumberOfRows(search); //총 자료의 갯수를 가져와서 변수에 저장
				...
				RequestDispatcher dis = request.getRequestDispatcher("HospitalList.jsp");
...
//		System.out.println(search);
		...
		HospitalDAO hDAO =HospitalDAO.getInstance();	
		List<HospitalDTO> data = hDAO.findStarList(currentPage, recordsPerPage, search);  //limit을 건 select 가져오기
		request.setAttribute("data",data); //가져온 자료를 담은 리스트를 리퀘스트로 전송
		
		AllDAO aDAO = AllDAO.getInstance();
		int row = aDAO.getNumberOfRows("hospital", search);
