...
@WebServlet("/ReadBye")
public class ReadByeServlet extends HttpServlet {
...
//		System.out.println(search);
		...
		List<ByeDTO> data = bDAO.findList(currentPage, recordsPerPage, search);  //limit을 건 select 가져오기
		request.setAttribute("data",data); //가져온 자료를 담은 리스트를 리퀘스트로 전송
		
		AllDAO aDAO = AllDAO.getInstance();
		int row = aDAO.getNumberOfRows("bye", search);
		
//		int row = bDAO.getNumberOfRows(search); //총 자료의 갯수를 가져와서 변수에 저장
		...
		RequestDispatcher dis = request.getRequestDispatcher("byeList.jsp");
